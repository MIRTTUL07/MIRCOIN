...
def mine_block():
    ...
    blockchain.add_transaction(sender=node_address, receiver='Mirttul', amount=1)
    block=blockchain.create_block(proof, previous_hash)
    response={'message':"Congrats",
              'index':block['index'],
              'tinestamp':block['timestamp'],
              'proof':block['proof'],
...
def connect_node():
    ...
    response={'messasge':'All the nodes are connected ',
...
def replace_chain():
    ...
    if is_chain_replaced:
        response={'message':"Chain is replaced with largest one ",
